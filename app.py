...
st.title("📋 永利分隊勤務自動排班系統 (進階版)")
st.caption("已整合動態人員名單編輯功能")
...
with tab1:
    ...
    current_names = [p['姓名'] for p in current_personnel if str(p['姓名']) != 'None']
    ...
    # --- 初始化今日可用人力池與狀態追蹤 ---
    hours_tracker = {p['姓名']: 0 for p in current_personnel if str(p['姓名']) != 'None'}
    consecutive_tracker = {p['姓名']: 0 for p in current_personnel if str(p['姓名']) != 'None'}
    history_91_count = {p['姓名']: 0 for p in current_personnel if str(p['姓名']) != 'None'}
    ...
    for idx, slot in enumerate(time_slots):
        ...
        for p in current_personnel:
            ...
            if pd.isna(name) or name == 'None': continue
